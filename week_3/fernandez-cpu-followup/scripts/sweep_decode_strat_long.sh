...
# NOTE: 128 tokens was tested first and found insufficient — none of the 4
# strategies completed the bubble sort function in that budget (all cut off
# mid-loop, no return statement). Bumped to 256 so completion is possible.
FIXED_INPUT=512
FIXED_OUTPUT=256
...
echo "strategy,input_tokens_actual,output_tokens_actual,contains_return,prefill_tokens_per_sec,decode_tokens_per_sec,cpu_freq_khz,timestamp,run,energy_joules,wall_seconds" > "$OUT"
...
# Rough completion heuristic: does the generated text contain a return
# statement? Not a correctness check (doesn't verify the code actually
# works), just a cheap signal for "did generation reach a natural
# conclusion" vs. "got cut off mid-implementation" — worth spot-checking
# manually either way before trusting it as ground truth.
check_completion() {
  local text=$1
  if echo "$text" | grep -q "return"; then
    echo "1"
  else
    echo "0"
  fi
